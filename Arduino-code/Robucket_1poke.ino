...
int deliverSugar() {
  lcd.setCursor(2, 1);            // move the LCD cursor to row 2, column 5
...
void setup() {                
  ...
  File dataFile = SD.open("RoBucket.csv", FILE_WRITE);
  if (dataFile) {
    dataFile.println("Program,Time,Sugar Deliveries,Active Pokes,Well Entries");
    dataFile.close();
    }
  }
}
...
void loop() { 
  ...
  drinkState = digitalRead(drinkPin);
    ...
    if(currentMillis - previousMillis > 1000)             // if the specified time interval has passed...
    {
      ...
      lcd.setCursor(9,0);                                    // bring the cursor to the first row, column 11
